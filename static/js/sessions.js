...
let _sortMode = Storage.get('aivox-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'AiVox Chat';
...
const FOLDER_STATE_KEY = 'aivox-folder-state';
const FOLDER_ORDER_KEY = 'aivox-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to AiVox kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'AiVox Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('aivox-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('aivox-session-sort', mode);
  else Storage.remove('aivox-session-sort');
  renderSessionList();
}
