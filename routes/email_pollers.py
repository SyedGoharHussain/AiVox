                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in AiVox (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in AiVox" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "aivox_kind" if "aivox_kind" in cols else "'scheduled' AS aivox_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                aivox_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-AiVox-Origin"] = "aivox-ui"
                outer["X-AiVox-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", aivox_kind or "scheduled")[:64]
                outer["X-AiVox-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `aivox-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `AIVOX_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `aivox-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("AIVOX_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `AIVOX_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (AIVOX_INPROCESS_POLLERS=0); "
            "drive `aivox-mail poll-scheduled` externally."
        )
        return
    import asyncio
